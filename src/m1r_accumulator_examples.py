...
#
#              for k in range(r, s): <- used a hyphen instead of a comma
#                  count = 0 <- initialized count inside the loop
#                  if math.cos(k) > 0: <- used r instead of k
#                      count = count + 1 <- used 1 instead of count + 1
#                  return count
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean? <- it's where "k" would go
#
